...
presence (`main/isobus/ecu_identity.cpp`), and Phase 3 minimal VT presence
(`main/isobus/vt_app.cpp` + `main/isobus/object_pool.iop`): blinks the
WS2812 status LED, toggles relay channel 1 through the TCA9554PWR I2C
expander, reads digital input 1, runs a CAN/TWAI self-test loopback, hands
the TWAI peripheral to AgIsoStack++ to claim an ISO 11783-5 NAME/address,
then uploads a VT object pool (8-in-a-row relay indicators + a 9-key Soft
Key Mask) and wires SK1-SK8 to the relays and SK9 to the buzzer. No AUX-N
yet (Phase 4) -- channels can only be worked from the VT screen so far, not
a joystick/armrest button.
...
VT client + object pool) exist so far; `automation/`, `config/`, and
`net/` land in later phases. `isobus/auxn_app.cpp` is next (Phase 4).
